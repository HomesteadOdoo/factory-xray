(() => {
  ...
  function wireFacilityDrill(panel){
    panel.querySelectorAll('[data-facility-id]').forEach(btn=>btn.addEventListener('click',()=>{
      const id=btn.dataset.facilityId;
      if(!id)return;
      if(typeof window.loadDetail==='function') window.loadDetail('facility',id,'geography');
      else location.hash='#facilities';
    }));
  }
